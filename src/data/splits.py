...
def create_patient_splits(
    ...
    print("\n SPLIT SIZES:")
    print(
        f"  Train: {len(train_pids):>5} patients ({len(train_pids)/len(labels_filtered)*100:.1f}%)"
    )
    print(
        f"  Val:   {len(val_pids):>5} patients ({len(val_pids)/len(labels_filtered)*100:.1f}%)"
    )
    print(
        f"  Test:  {len(test_pids):>5} patients ({len(test_pids)/len(labels_filtered)*100:.1f}%)"
    )
