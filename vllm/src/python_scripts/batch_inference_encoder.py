    ...
    async with semaphore:
        try:
            response = await client.embeddings.create(
                model=args.model, input=final_input, timeout=60
            )
...
async def main(args):
    client = AsyncOpenAI(
        base_url=f"http://localhost:{args.port}/v1", api_key="vllm-embed"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", type=str, required=True, help="Input .jsonl with 'pid' and 'report'"
    )
    ...
    parser.add_argument(
        "--task_description", type=str, default="Retrieve relevant medical passages"
    )
